...
def name_to_abrev(name):
    return state_name_to_abrev[name.lower()]
...
def calc_tax_info(us_state, income, marital_status='single', dependents=0):
    ...
    if us_state.lower() in state_name_to_abrev:
        us_state = name_to_abrev(us_state)
    assert us_state in state_map
